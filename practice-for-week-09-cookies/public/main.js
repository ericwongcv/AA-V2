...
function storeTheme(themeName) {
    // Your code here
    const themeNameCookie = "themeName=" + themeName;
    document.cookie = themeNameCookie;
}
...
function restoreTheme() {
    ...
    if (document.cookie.split("=").length > 2) {
        themeName = document.cookie
            .split('; ')
            .find(row => row.startsWith('themeName='))
            .split('=')[1];
    } else {
        const themeNameKeyValue = document.cookie.split("=");
        if (themeNameKeyValue[0] === 'themeName') themeName = themeNameKeyValue[1];
    }
...
function storeName(displayName) {
    // Your code here
    const displayNameCookie = "displayName=" + displayName;
    
    document.cookie = displayNameCookie;
}
...
function restoreName() {
    ...
    if (document.cookie.split("=").length > 2) {
        displayName = document.cookie
            .split('; ')
            .find(row => row.startsWith('displayName='))
            .split('=')[1];
    } else {
        const displayNameKeyValue = document.cookie.split("=");
        if (displayNameKeyValue[0] === "displayName") displayName = displayNameKeyValue[1];
    }
