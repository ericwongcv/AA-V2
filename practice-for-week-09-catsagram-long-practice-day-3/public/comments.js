...
const submitComment = e => {
    ...
    // if stored comment exists push new comment into array. Else, create new array with comment
    let storedComments = localStorage.getItem('comments');
    storedComments ? localStorage.setItem('comments', storedComments += "/break/" + commentText)
        : localStorage.setItem('comments', commentText);
...
const loadComments = () => {
    ...
    comments.split("/break/").forEach( commentText => {
        createComment(commentText);
    });
};
