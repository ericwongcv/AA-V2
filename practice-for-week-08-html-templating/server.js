...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      ...
      let liDogs = dogs.map( dog => `<li>${dog.name}</li>`).join('');

      const htmlPage = htmlTemplate.replace(/#{dogsList}/g, liDogs);
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        let htmlPage = htmlTemplate.replace(/#{name}/g, dog.name).replace(/#{age}/, dog.age);
    ...
    if (req.method === 'POST' && req.url === '/dogs') {
      // Your code here
      const dogInfo = reqBody.split('&').map(pair => pair.split('='));
      let dogName = dogInfo[0][1];
      let dogAge = dogInfo[1][1];
